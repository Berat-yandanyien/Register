...
if(!["787937477993955368"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) return message.channel.send(`Bu Komutu Kullanabilmek İçin Yetkin Bulunmuyor.`)


const member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));
if(!member) return message.channel.send(`**<a:Cros:788055287768678421>KAYDINI SIFIRLAMAM GEREKEN KULLANICIYI BELİRT<a:Cros:788055287768678421>**`)
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ["sıfırla", "kayıt-sıfırla", "kayıtları-sıfırla", "ks", "dbisil", "db-sil"],
    permLevel: 0
};
